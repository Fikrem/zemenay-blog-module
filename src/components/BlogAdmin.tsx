...
rows={16}
      ...
      {showImageModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Add Image</h3>
            <input
              type="url"
              value={imageUrl}
      ...
      {showVideoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Add Video</h3>
            <input
              type="url"
              value={videoUrl}
  ...
  return (
    <div className="container mx-auto p-4 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="animate-pulse space-y-6">
        ...
        <div className="space-y-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="p-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
...
export function BlogAdmin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        if (!user) {
          showToast("No authenticated user found. Please sign in again.", "error");
          setSubmitting(false);
          return;
        }
        const postData = { 
  ...
  // ===== MAIN RENDER =====
  return (
    <div className="container mx-auto p-4 max-w-7xl min-h-screen" style={{ backgroundColor: 'var(--bg-secondary)' }}>
      {/* Toast Notifications */}
      {toast && (
        <Toast
  ...
  <div>
    ...
    <p className="mt-1" style={{ color: 'var(--text-secondary)' }}>Manage your blog posts</p>
  </div>
  <div className="flex items-center gap-4">
    <ThemeSwitcher />
    <button
    ...
    >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
        ...
        <div className="shadow-lg rounded-lg p-6 border" style={{ 
  backgroundColor: 'var(--bg-primary)', 
  borderColor: 'var(--border-color)' 
}}>
...
style={{ 
  ...
  color: 'var(--text-primary)' 
              ...
              {/* Slug Input */}
              <div>
                <label htmlFor="slug" className="block text-sm font-medium  mb-2" style={{ color: 'var(--text-primary)' }}>
                  Slug
                </label>
                <input
                  id="slug"
...
style={{ 
  ...
  color: 'var(--text-primary)' 
                  ...
                  disabled={submitting}
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Auto-generated from title, but you can customize it</p>
              </div>

              {/* Author Name Input */}
              <div>
                <label htmlFor="authorName" className="block text-sm font-medium mb-2" style={{ color: 'var(--text-primary)' }}>
                  Author Name
                </label>
                <input
                  id="authorName"
